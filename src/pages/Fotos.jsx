...
export default function Fotos({ business, onUpdate }) {
  ...
  async function fetchGallery() {
    const { data } = await supabase
      .from('business_images').select('*').eq('business_id', business.id).order('order')
  ...
  async function uploadLogo(e) {
    ...
    const { error: upError } = await supabase.storage.from('businesses').upload(path, file, { upsert: true })
    if (upError) {
  ...
  async function uploadGalleryPhoto(e) {
    ...
    const { error: upError } = await supabase.storage.from('businesses').upload(path, file, { upsert: false })
    if (upError) {
    ...
    await supabase.from('business_images').insert({
      business_id: business.id, url: publicUrl, order: galleryImages.length,
    })
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-heading font-extrabold text-brand-navy text-2xl mb-1">Logo y fotos</h1>
        <p className="font-body text-gray-500 text-sm">Tu logo aparece en tu tarjeta del directorio. La galería se muestra en tu perfil (máx. 8 fotos).</p>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm mb-6">
        <h2 className="font-heading font-bold text-brand-navy text-base mb-4">Logo</h2>
        <div className="flex items-start gap-6">
          {business.logo_url ? (
            <img src={business.logo_url} alt="Logo" className="w-32 h-32 rounded-xl object-cover border border-gray-100" />
          ) : (
            <div className="w-32 h-32 rounded-xl bg-gray-100 flex items-center justify-center text-4xl">🏪</div>
          )}
          <div>
            <p className="font-body text-gray-500 text-sm mb-3">Formato JPG o PNG. Recomendado: imagen cuadrada.</p>
            ...
            >
              {logoUploading ? 'Subiendo...' : business.logo_url ? 'Cambiar logo' : 'Subir logo'}
            </button>
          </div>
        </div>
      </div>
